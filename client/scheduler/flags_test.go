	...
	for cpuClass := range scheduler_proto.CPUClass_name {
		for gpuClass := range scheduler_proto.GPUClass_name {
			for diskClass := range scheduler_proto.DiskClass_name {
				for reschedulePolicy := range scheduler_proto.Job_ReschedulePolicy_name {
					for restartPolicy := range scheduler_proto.Job_RunConfig_RestartPolicy_name {
						...
						assert.Equal(t, scheduler_proto.Job_RunConfig_RestartPolicy(flags.RestartPolicy),
							job.RunConfig.RestartPolicy)
						assert.Equal(t, scheduler_proto.Job_ReschedulePolicy(flags.ReschedulePolicy),
							job.ReschedulePolicy)
