...
// TaskTerminatedHandler updates the corresponding Container status and the corresponding Node's resources.
...
// Handle updates the status of the corresponding Container and the corresponding Node's available resources.
func (s *TaskTerminatedHandler) Handle(eventProto *events_proto.Event) error {
	...
	tx := s.db.Begin()
	// Load the corresponding Container to check if it needs to be rescheduled.
	...
	// Check if the Container needs to be rescheduled on Task failure.
	if task.Job.NeedsRescheduling(scheduler_proto.Task_Status(task.Status)) {
		// Make the Container available for scheduling.
		newJobStatus = scheduler_proto.Job_STATUS_PENDING
	}

	jobStatusUpdatedEvent, err := task.Job.UpdateStatus(tx, newJobStatus)
	if err != nil {
		return utils.RollbackTransaction(tx, errors.Wrap(err, "failed to update Container status"))
	}

	node := &models.Node{Model: utils.Model{ID: task.NodeID}}
	// Deallocate the Container's resources from the Node.
	nodeUpdates := node.DeallocateContainerLimit(task.Job)
	...
	nodeUpdatedEvent, err := node.Update(tx, nodeUpdates)
	if err != nil {
	...
	if err := utils2.CommitAndPublish(tx, s.producer, jobStatusUpdatedEvent, nodeUpdatedEvent); err != nil {
